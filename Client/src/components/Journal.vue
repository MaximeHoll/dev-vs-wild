<template>
  ...
  <div class="flex w-full justify-center items-center">
    <div class=" w-[95vw] h-[65vh] text-center bg-greenh rounded-2xl z-50 bg-opacity-80 p-4 overflow-y-scroll">
      <h1 class="text-center text-4xl">New Entry</h1>
      <div class="text-center">
      <form id="newEntry" v-on:submit.prevent="addEntry">
        <input type="text" name="entry" id="entry" v-model="formEntry" required/>
        <br>
        <button type="submit">New Entry</button>
      </form>
      </div>
      <h2 class="text-center text-4xl">Journal</h2>
      <div v-for="entry in entries" class="mb-2 opacity-70 bg-white rounded-lg" vm-forceUpdate>
        <p class="text-xs text-right italic mr-2">{{moment(entry.creation_date).format('DD-MM-YYYY, h:mm:ss a')}}</p>
